...
export type CommandHandlerParams = NarrowedContext<Context<Update>, { message: Update.New & Update.NonChannel & Message.TextMessage; update_id: number }>
...
export function replyWithSticker(ctx: CommandHandlerParams, sticker: string) {
...
export function extractLinkFromMessage(message: Message.TextMessage) {
 ...
 */
export function checkMessageWithLink(message: Message.TextMessage): { newLink?: string, oldLink?: DBEntry, err: boolean } {
  ...
  return { newLink: newLink, oldLink: links.find((e: DBEntry) => (e.host == host && e.path == pathname)), err: false }
}